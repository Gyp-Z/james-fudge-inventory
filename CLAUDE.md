...
- **DONE FOR THE SEASON — Lisa's Sept 2026 closeout "done making" list (`PAUSED_FLAVORS`/`isPaused` in `src/core/ops.js`): Maple Walnut, Chocolate M&M, Chocolate Reese's, Dirt, Vanilla Marshmallow, Chocolate Mint, Chocolate Coconut, Key Lime, Chocolate Rocky Road, Chocolate Raspberry, Vanilla Chocolate Chip.** Stop producing — let existing stock sell down to zero. This is the "what to slow down on" list. Rolls up the earlier July/Aug pauses (Key Lime, Van Choc Chip, Choc Rocky Road, Choc Coconut, Choc Raspberry) PLUS the Sept cuts. **Maple Walnut is a fall seller but was stopped EARLY** — it's made 6-trays-at-a-time and 6 were still on hand, so Lisa cut it (this is why it's here, not in as-needed, despite the "fall sellers keep going" idea). `getMakeRecommendations` skips them; `getSeasonOutlook` gives them `done: true` + verdict `'done'`; Jarvis system prompt has the "DONE FOR THE SEASON" rule (never recommend; only make if one hits zero AND Zach cleared it with Lisa). Pausing only gates recommendations — logging/deductions/base reminders still work if one gets made. Not a schema flag — name set + prompt.
- **MAKE AS NEEDED — Lisa's keep-in-rotation list (`AS_NEEDED_FLAVORS`/`isAsNeeded` in `src/core/ops.js`): Chocolate Marshmallow, Cookies & Cream, Snickerdoodle, Pistachio, Pumpkin Spice, Vanilla M&M.** Still sell but can get LOW — make more when they run down. Supersedes the old summer `FALL_FLAVORS` concept (removed): `getSeasonOutlook` gives them `as_needed: true` and NEVER a `'stop'` verdict — they `'coast'`, or `'make_small'` when they'll run dry before close. They're not paused, so they surface in `getMakeRecommendations` when low. Jarvis system prompt has the "MAKE AS NEEDED" rule. The conceptual opposite of the done list. Not a schema flag — name set + prompt.
- **Flavors on NEITHER list (Vanilla / Chocolate / Peanut Butter bases, both Sea Salt Caramels, Vanilla Walnut, Chocolate Walnut, Chocolate PB):** keep making to demand as usual — they fall through to `getSeasonOutlook`'s default sell-down logic (stop if projected leftover > 1, else make_small/coast). Bases especially must never run low (they feed the other flavors — the system prompt says so).
...
**`getSeasonOutlook(sb, { window, asOf })`** is the threshold-free wind-down brain: per fudge flavor it projects, from real recent sales, days of stock left, sellout date, and **projected leftover trays at close** (the waste forecast to drive to zero), plus a verdict (`stop` / `coast` / `make_small` / `done`). `make_small` ("worth making") is reserved for **top sellers** that run dry with a real safety margin before close — by default top ~30% by sell-rate, ≥7 days margin. Pumpkin Spice (late-Oct closeout flavor) is excluded. Popcorn is returned separately with `make_fresh_to_demand: true` — it is **never** part of the fudge sell-down (short shelf life → made fresh to demand right up to close).

**Pace-aware tightening (`PRIOR_YEAR_REFERENCE`, added Sept 2026):** the owner's mom recalled ~140 total fudge trays on the shelf around this same date in 2025 — an anecdotal but real benchmark. `getSeasonOutlook` compares it to today's actual total fudge trays and returns `prior_year_reference` (the numbers) + `pace` (`overstocked_vs_last_year`, the top-seller percentile and safety-margin days actually in effect). When we're carrying >10% more stock than that point last year, the `make_small` bar auto-tightens from top-30%/7-day-margin to top-10%/14-day-margin — fewer, more genuinely urgent recommendations, with zero extra logic needed by callers. This is a one-time historical data point, not day-by-day history — never used for anything but this one tightening.

This one core fn feeds the `get_season_outlook` Jarvis tool, the **Analytics "Season Outlook" panel** (`SeasonOutlookPanel` in `Analytics.jsx` — leads with a short urgency-sorted "What to make" list, not the full per-flavor table, which is now tucked behind "Show full breakdown"), and `getMakeRecommendations`, which in wind-down **delegates its fudge inclusion directly to this verdict** (`include = verdict === 'make_small'`) so the two surfaces never disagree — an empty/short wind-down fudge list is the expected, correct state, not a gap.

**Wind-down alert sync (`syncWindDownThresholds` in `src/core/ops.js`, owner-requested Sept 2026) — supersedes the old "thresholds are never auto-edited" rule.** `low_tray_threshold` is now the ONE place every consumer (Dashboard, Admin, `get_low_stock`, MCP) reads "is this flavor low" from, so rather than each needing its own wind-down special-case, this function makes the column itself carry the wind-down-aware answer:
- **In winddown/closed:** for each active fudge flavor, snapshots the owner's real threshold into `flavors.peak_low_tray_threshold` the FIRST time it runs (never overwritten again until restored), then sets `low_tray_threshold` to the flavor's current on-hand count if its verdict is `make_small` (so it reads as "low" right now), or zeroes it (stops nagging) only if verdict is `stop` (explicitly overstocked) or `done` (paused) — the group we actually WANT to run to zero with no alert. A `coast` verdict (roughly balanced, not currently flagged either way) keeps the real peak threshold instead of zeroing IF the flavor is a base or a real top seller (see the tier exception below) — a top seller running a bit low earlier than the sales projection expected still deserves a live nudge. A coasting non-top-tier specialty flavor is the one case that zeroes (fine/expected to run to zero that late). Idempotent — safe to call many times a day.
- **Exception — base flavors AND real top sellers never get zeroed while coasting.** `is_base_trigger` flavors (Vanilla, Chocolate, Peanut Butter) feed other flavors and this app's rule is they must NEVER run low, wind-down included (see the base-group reminder system above). The same protection extends to any flavor whose `peak_low_tray_threshold >= 3` (the "strong seller" tier from the peak-threshold curation — e.g. Chocolate Peanut Butter, both Sea Salt Caramels, Cookies & Cream): a `coast` verdict keeps the real peak-season threshold instead of 0; only `stop`/`done` (or, for anyone, a genuine `make_small`) override that. Discovered/fixed Sept 2026 in two passes: first after the initial sync zeroed Vanilla/Chocolate/Peanut Butter's alerts along with every specialty flavor (fixed: bases exempted), then again after Chocolate Peanut Butter — a 0.92 trays/day peak seller, nearly base-level — still zeroed out while merely `coast` (fixed: the tier exception + `stop`/`done`-only zeroing above).
- **Peak thresholds are data-curated, not guessed.** `scripts/curate-peak-thresholds.mjs` (kept for reference/reruns) computes each fudge flavor's real peak-window (`SEASON_START`..`fudgeWinddownMonthDay`) sell rate from `shift_report_entries` and assigns `peak_low_tray_threshold` by tier: bases get `round(rate*3)` floored at 3 (Vanilla/Chocolate → 4, Peanut Butter → 3); non-base "strong sellers" (peak rate ≥ 0.5/day) get 3; everyone else with real peak sales gets 2 (the app's long-standing universal default); zero peak-window sales (e.g. Pumpkin Spice, added after peak started) gets 0. A flat linear formula was tried first and rejected — it collapsed every strong non-base seller down to the same "2" floor as flavors selling 5-10x slower, which is what surfaced the Chocolate Peanut Butter issue above.
- **In peak/preseason:** restores `low_tray_threshold` from `peak_low_tray_threshold` for any flavor that has a snapshot, then clears it — a no-op once already restored. This is how next season's peak gets the owner's real numbers back untouched, including any Admin edits made during THAT peak (only the FIRST wind-down of a season captures the snapshot).
- Exposed as the `sync_alert_thresholds` Jarvis/MCP tool, and auto-called from `Dashboard.jsx`'s and `Admin.jsx`'s wind-down effect on every page load, BEFORE flavors are considered loaded (no cron infra needed — the tablet gets opened daily in practice; blocking on it avoids a stale-then-flip render). Admin.jsx shows a small "peak was N" pill next to any threshold currently overridden this way.
- Manual Admin edits made mid-wind-down get overwritten by the next sync — that's intentional; the smart model is authoritative for `low_tray_threshold` during this window (except bases, which just keep their peak number as described above).
- **`flavors` table RLS gap (discovered Sept 2026):** `flavors` was missing anon INSERT/UPDATE policies entirely (only ever granted to `authenticated`, plus a later anon-SELECT-only migration) — `supabase/migrations/add_flavors_anon_write_policies.sql` fixes it. Before that fix, Admin.jsx's `saveThreshold`/`toggleActive`/`handleAdd` were silently no-op'ing in production (RLS-blocked UPDATEs return 0 rows with no error — the same failure mode documented in Common Pitfalls below for the revert helpers). If flavor edits ever seem to silently not save again, check this policy first.
...
- **Partially live (wind-down only) as of Sept 2026** — see `syncWindDownThresholds` above. Peak-season threshold tuning from accumulated sales data is still open.
